...
ML.createResults = function (containerID, config) {
    ...
    // For array of snippet objects, get the value corresponding to a key
    getSnip = function (key, data) {
        var result = '';
        for (var m in data) {
            if (data[m][key] !== undefined) {
                result = data[m][key];
    ...
    if(!data.boundedbox){			
        ...
        container.html(
                '<div id="results-header" style="position:absolute;overflow:none">'
        );
        ...
        for (var i=0; i < data['results'].length; i++) {
                        ...
                        for ( var j in data['results'][i].matches[k]['match-text']) {
                            ...
                            if (data['results'][i].matches[k]['match-text'][j] instanceof Object) {
                                longSnippet += '<span class="highlight">'
                                        + encodeTags(data['abstract-snippet'][i].matches[k]['match-text'][j]['highlight'])
                                        + '</span>';
            ...
            try {            
                snipContent = '';
                // Ensure metadata is an array
                abstractSnip = wrapArray(config.metadata.columns[0]['abstract-snippet']);
                for (var c in abstractSnip) {
                    // literal is user-defined text
                    if (abstractSnip[c]['type'] === 'literal') {
                        snipContent += '<span>' + abstractSnip[c]['text'] + '</span>';
                    // value is from the document XML
                    } else if (abstractSnip[c]['type'] === 'value') {
                        var key = '';
                        if (abstractSnip[c]['attr']) {
                            // for attribute range indexes, key is element and attribute names concatenated with "_"
                            key = abstractSnip[c]['elem'] + '_' + abstractSnip[c]['attr'];
                        } else {
                            // for element range indexes (attr property does not exist), key is element name
                            key = abstractSnip[c]['elem'];
                        }
                        className = key.replace(/[^_a-z0-9-]/gi, '_'); // Ensure safe class name
                        snipContent += '<span class="ml-' + className + '">' + getMeta(key, meta) + '</span>';
                    }
<!--meta, getMeta, abstractMeta -->
            ...
            content += '<span class="title"><a href="/v1/documents?uri=' + encodeURIComponent(data['results'][i].uri) + '" target="purple">' + titleContent + '</a></span>';
